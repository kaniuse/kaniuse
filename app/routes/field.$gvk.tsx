...
export default function FieldGVK() {
  ...
  const versions = Array.from(new Set(fieldSummary.flatMap(field =>
    field.LifeCycles.map(lc => lc.KubernetesMinorRelease)
  ))).sort((a, b) => {
          ...
          <thead>
            <tr>
              ...
              {versions.map(version => (
                <th key={version} className="border px-4 py-2">{version}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {fields.map(field => (
              <tr key={field}>
                <td className="border px-4 py-2">
                  {field
                    .replace('io.k8s.api.', '')
                    .replace((gvk as string).split(' ')[0].replace('/', '.') + '.', '')}
                </td>
                {versions.map(version => {
                  ...
                  return (
                    <td
                      ...
                      className={`border px-4 py-2 ${
                        lifecycle === 'stable' ? 'bg-green-500' :
                        lifecycle === 'deprecated' ? 'bg-yellow-500' :
                        'bg-neutral'
                      }`}
                    >
                      {lifecycle}
