...
export default function KindDetail() {
  ...
  const versions = Array.from(new Set(data.flatMap((gvk) => gvk.Lifecycles.map((l) => l.kubernetesMinorRelease)))).sort(
    (a, b) => {
          ...
          {spans.map((span, spanIndex) => (
            ...
            >
              {span.spans.map((item, itemIndex) => (
                <div
                  key={itemIndex}
                  className={`flex border-neutral ${itemIndex !== span.spans.length - 1 ? 'border-l' : 'border-x'}`}
                  style={{
                    gridColumn: `${versions.indexOf(item.start) + 1} / ${versions.indexOf(item.end) + 2}`,
                  }}
                >
                  <div
                    className={`w-full h-4 my-auto ${item.lifecycle === 'stable' ? 'bg-green-500' : 'bg-yellow-500'}`}
                  ></div>
                </div>
              ))}
            </div>
